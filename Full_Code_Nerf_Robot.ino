...
// Filtering and deadband variables
const int numSamples = 10;  // Number of samples for averaging
int xSamples[numSamples];   // Array to store X-axis samples
int zSamples[numSamples];   // Array to store Z-axis samples
int sampleIndex = 0;        // Index for the current sample
float alpha = 0.2;          // Smoothing factor for low-pass filter
int16_t filteredX = 0, filteredZ = 0;
int deadband = 100;         // Deadband for sensor readings
...
void controlAiming() {
    ...
    int xAvg = xSum / numSamples;
    int zAvg = zSum / numSamples;

    // Apply deadband
    if (abs(xAvg - filteredX) < deadband) xAvg = filteredX;
    if (abs(zAvg - filteredZ) < deadband) zAvg = filteredZ;

    // Apply low-pass filter
    filteredX = alpha * xAvg + (1 - alpha) * filteredX;
    filteredZ = alpha * zAvg + (1 - alpha) * filteredZ;

    // Map filtered sensor values to servo movement
    int servoXAngle = map(filteredZ, SENSOR_MIN, SENSOR_MAX, SERVO_MIN, SERVO_MAX);
    int servoYAngle = map(filteredX, SENSOR_MIN, SENSOR_MAX, SERVO_MIN, SERVO_MAX);
...
void controlFiring() {
    ...
    if (buttonState == LOW) {  // If button is pressed
        fireServo.write(90);   // Move servo to 90° to fire
        digitalWrite(CONTROL_PIN, HIGH);  // Activate trigger
        Serial.println("Firing");
        delay(500);
        fireServo.write(0);  // Return to default position
